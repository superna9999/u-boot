...
static int meson_pcie_get_link_speed(struct meson_pcie *priv)
{
	return (readl(priv->dbi_base + PCIE_LINK_STATUS_REG) &
		PCIE_LINK_STATUS_SPEED_MASK) >> PCIE_LINK_STATUS_SPEED_OFF;
}

static int meson_pcie_get_link_width(struct meson_pcie *priv)
{
	return (readl(priv->dbi_base + PCIE_LINK_STATUS_REG) &
		PCIE_LINK_STATUS_WIDTH_MASK) >> PCIE_LINK_STATUS_WIDTH_OFF;
}

static void meson_pcie_writel_ob_unroll(struct meson_pcie *priv, u32 index,
				     u32 reg, u32 val)
{
	u32 offset = PCIE_GET_ATU_OUTB_UNR_REG_OFFSET(index);
	void __iomem *base = priv->dbi_base;

	writel(val, base + offset + reg);
}

static u32 meson_pcie_readl_ob_unroll(struct meson_pcie *priv, u32 index, u32 reg)
{
	u32 offset = PCIE_GET_ATU_OUTB_UNR_REG_OFFSET(index);
	void __iomem *base = priv->dbi_base;
...
static inline void meson_pcie_dbi_write_enable(struct meson_pcie *priv, bool en)
{
	u32 val;

	val = readl(priv->dbi_base + PCIE_MISC_CONTROL_1_OFF);

	if (en)
		val |= PCIE_DBI_RO_WR_EN;
	else
		val &= ~PCIE_DBI_RO_WR_EN;
	writel(val, priv->dbi_base + PCIE_MISC_CONTROL_1_OFF);
}

/**
 * meson_pcie_setup_host() - Setup the PCIe controller for RC opertaion
 *
 ...
 */
static void meson_pcie_setup_host(struct meson_pcie *priv)
{
	meson_pcie_dbi_write_enable(priv, true);

	/* setup RC BARs */
	writel(PCI_BASE_ADDRESS_MEM_TYPE_64,
	       priv->dbi_base + PCI_BASE_ADDRESS_0);
	writel(0x0, priv->dbi_base + PCI_BASE_ADDRESS_1);

	/* setup interrupt pins */
	clrsetbits_le32(priv->dbi_base + PCI_INTERRUPT_LINE,
			0xff00, 0x100);

	/* setup bus numbers */
	clrsetbits_le32(priv->dbi_base + PCI_PRIMARY_BUS,
			0xffffff, 0x00ff0100);
	...
	setbits_le32(priv->dbi_base + PCIE_LINK_WIDTH_SPEED_CONTROL,
		     PORT_LOGIC_SPEED_CHANGE);

	meson_pcie_dbi_write_enable(priv, false);
}
...
/**
 * meson_pcie_dw_prog_outbound_atu_unroll() - Configure ATU for outbound accesses
 *
 * @meson_pcie: Pointer to the PCI controller state
 ...
 */
static int meson_pcie_prog_outbound_atu_unroll(struct meson_pcie *priv, int index,
					    int type, u64 cpu_addr,
					    u64 pci_addr, u32 size)
{
	u32 retries, val;

	dev_dbg(priv->dev, "ATU programmed with: index: %d, type: %d, cpu addr: %8llx, pci addr: %8llx, size: %8x\n",
		index, type, cpu_addr, pci_addr, size);

	meson_pcie_writel_ob_unroll(priv, index, PCIE_ATU_UNR_LOWER_BASE,
				 lower_32_bits(cpu_addr));
	meson_pcie_writel_ob_unroll(priv, index, PCIE_ATU_UNR_UPPER_BASE,
				 upper_32_bits(cpu_addr));
	meson_pcie_writel_ob_unroll(priv, index, PCIE_ATU_UNR_LIMIT,
				 lower_32_bits(cpu_addr + size - 1));
	meson_pcie_writel_ob_unroll(priv, index, PCIE_ATU_UNR_LOWER_TARGET,
				 lower_32_bits(pci_addr));
	meson_pcie_writel_ob_unroll(priv, index, PCIE_ATU_UNR_UPPER_TARGET,
				 upper_32_bits(pci_addr));
	meson_pcie_writel_ob_unroll(priv, index, PCIE_ATU_UNR_REGION_CTRL1,
				 type);
	meson_pcie_writel_ob_unroll(priv, index, PCIE_ATU_UNR_REGION_CTRL2,
				 PCIE_ATU_ENABLE);
	...
	for (retries = 0; retries < LINK_WAIT_MAX_IATU_RETRIES; retries++) {
		val = meson_pcie_readl_ob_unroll(priv, index,
	...
	dev_err(priv->dev, "outbound iATU is not being enabled\n");

	return -1;
}

/**
 * meson_pcie_dw_addr_valid() - Check for valid bus address
 *
 ...
 */
static int meson_pcie_addr_valid(pci_dev_t d, int first_busno)
{
 ...
 */
static uintptr_t set_cfg_address(struct meson_pcie *priv,
				 pci_dev_t d, uint where)
{
	int rel_bus = PCI_BUS(d) - priv->first_busno;
	...
	if (!rel_bus) {
		va_address = (uintptr_t)priv->dbi_base;
		goto out;
	}
	...
	ret = meson_pcie_prog_outbound_atu_unroll(priv, PCIE_ATU_REGION_INDEX1,
					       atu_type, (u64)priv->cfg_base,
					       d << 8, priv->cfg_size);
	if (ret)
		return (uintptr_t)ret;

	va_address = (uintptr_t)priv->cfg_base;
...
/**
 * meson_pcie_rd_conf() - Read from configuration space
 *
 ...
 * Return: 0 on success
 */
static int meson_pcie_rd_conf(const struct udevice *bus, pci_dev_t bdf,
				 uint offset, ulong *valuep,
				 enum pci_size_t size)
{
	struct meson_pcie *priv = dev_get_priv(bus);
	uintptr_t va_address;
	ulong value;
	...
	if (!meson_pcie_addr_valid(bdf, priv->first_busno)) {
		debug("- out of range\n");
		*valuep = pci_get_ff(size);
		return 0;
	}

	va_address = set_cfg_address(priv, bdf, offset);

	value = readl(va_address);

	debug("(addr,val,size)=(0x%04x, 0x%08lx, %d)\n", offset, value, size);
	 ...
	 */
	if (PCI_BUS(bdf) == 0 && offset == PCI_CLASS_REVISION && size == 2) {
		value = (PCI_CLASS_BRIDGE_PCI << 16) | (value & 0xffff);
	} else if (PCI_BUS(bdf) == 0 && offset == PCI_CLASS_DEVICE && size == 2) {
		value = PCI_CLASS_BRIDGE_PCI;
	}

	debug("(addr,val,size)=(0x%04x, 0x%08lx, %d)\n", offset, value, size);
	*valuep = pci_conv_32_to_size(value, offset, size);

	return meson_pcie_prog_outbound_atu_unroll(priv,
...
/**
 * meson_pcie_wr_conf() - Write to configuration space
 *
 * @bus: Pointer to the PCI bus
 * @bdf: Identifies the PCIe device to access
 * @offset: The offset into the device's configuration space
 ...
 * Return: 0 on success
 */
static int meson_pcie_wr_conf(struct udevice *bus, pci_dev_t bdf,
...
{
	u32 val;
	u16 offset = dm_pci_find_capability(priv->dev, PCI_CAP_ID_EXP);
	int max_payload_size = meson_size_to_payload(size);

	meson_pcie_dbi_write_enable(priv, true);

	val = readl(priv->dbi_base + offset + PCI_EXP_DEVCTL);
	val &= ~PCI_EXP_DEVCTL_PAYLOAD;
	writel(val, priv->dbi_base + offset + PCI_EXP_DEVCTL);

	val = readl(priv->dbi_base + offset + PCI_EXP_DEVCTL);
	val |= PCIE_CAP_MAX_PAYLOAD_SIZE(max_payload_size);
	writel(val, priv->dbi_base + PCI_EXP_DEVCTL);
...
static int meson_pcie_probe(struct udevice *dev)
{
	...
	printf("PCIE-%d: Link up (Gen%d-x%d, Bus%d)\n",
	       dev_seq(dev), meson_pcie_get_link_speed(priv),
	       meson_pcie_get_link_width(priv),
	       hose->first_busno);
...
static const struct dm_pci_ops meson_pcie_ops = {
	.read_config	= meson_pcie_rd_conf,
	.write_config	= meson_pcie_wr_conf,
};

...
{
	struct meson_pwrc_vpu_priv *priv = dev_get_priv(power_domain->dev);
	int i, ret;
...
struct power_domain_ops meson_pwrc_vpu_ops = {
...
static const struct udevice_id meson_pwrc_vpu_ids[] = {                          
	{ .compatible = "amlogic,meson-gx-pwrc-vpu" },
	{ }                                                                     
};  

static int meson_pwrc_vpu_probe(struct udevice *dev)
{
	struct meson_pwrc_vpu_priv *priv = dev_get_priv(dev);
	u32 hhi_phandle;
	ofnode hhi_node;
	int ret;
...
U_BOOT_DRIVER(meson_pwrc_vpu) = {
	.name = "meson_gx_pwrc_vpu",
	.id = UCLASS_POWER_DOMAIN,
	.of_match = meson_pwrc_vpu_ids,
	.probe = meson_pwrc_vpu_probe,
	.ops = &meson_pwrc_vpu_ops,
	.priv_auto_alloc_size = sizeof(struct meson_pwrc_vpu_priv),
};
